...
const formatDateAndTimeInBackTest = (strategy) => {
  return {
    ...strategy.toObject(),
    backTestData: strategy.backTestData.map((data) => ({
      ...data.toObject(),
      date: dayjs(data.date).format("D MMM, YYYY"),
    })),
  };
};
...
const getStrategyByName = async (req, res) => {
  try {
    ...
    let strategy = await strategyService.getStrategyByName(name);
    strategy = formatDateAndTimeInBackTest(strategy);
    if (!strategy) {
